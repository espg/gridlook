<script setup lang="ts">
import { storeToRefs } from "pinia";
import { computed } from "vue";

import {
  resolveChoroplethRange,
  scanFeatureProperty,
} from "@/lib/layers/vectorChoropleth.ts";
import { getVectorLayerData } from "@/lib/layers/vectorDataStore.ts";
import type { TColorMap } from "@/lib/shaders/colormapShaders.ts";
import { LAYER_KINDS, useGlobeControlStore } from "@/store/store.ts";
import { formatValue } from "@/utils/formatValue.ts";

// One legend card per visible vector layer with an active choropleth,
// stacked above the distance scale. The gradient strips are the prerendered
// colormap images (the same ones HoverReadout samples), so no extra WebGL
// context is needed per card.
const store = useGlobeControlStore();
const { layerStack } = storeToRefs(store);

type TLegendEntry = {
  id: string;
  name: string;
  property: string;
  colormap: TColorMap;
  low: number;
  high: number;
};

const legendEntries = computed<TLegendEntry[]>(() => {
  const entries: TLegendEntry[] = [];
  for (const layer of layerStack.value) {
    const style = layer.vectorStyle;
    const data =
      layer.kind === LAYER_KINDS.VECTOR && layer.visible
        ? getVectorLayerData(layer.id)
        : undefined;
    if (!data || !style?.colorBy) {
      continue;
    }
    const range = resolveChoroplethRange(
      scanFeatureProperty(data, style.colorBy).autoRange,
      style
    );
    if (range) {
      entries.push({
        id: layer.id,
        name: layer.name,
        property: style.colorBy,
        colormap: style.colormap,
        ...range,
      });
    }
  }
  return entries;
});

function colormapImage(colormap: TColorMap) {
  return `${import.meta.env.BASE_URL}static/colormaps/${colormap}.webp`;
}
</script>

<template>
  <div v-if="legendEntries.length" class="vector-legend">
    <div
      v-for="entry in legendEntries"
      :key="entry.id"
      class="vector-legend-card"
    >
      <div class="vector-legend-title" :title="entry.name">
        {{ entry.name }}
      </div>
      <div class="vector-legend-property">{{ entry.property }}</div>
      <img
        class="vector-legend-gradient"
        :src="colormapImage(entry.colormap)"
        :alt="`${entry.colormap} colormap`"
      />
      <div class="vector-legend-labels">
        <span>{{ formatValue(entry.low) }}</span>
        <span>{{ formatValue(entry.high) }}</span>
      </div>
    </div>
  </div>
  <div v-else />
</template>

<style lang="scss" scoped>
// mirrors the VectorHoverReadout card language, pinned bottom-left above the
// distance scale so it never collides with the corner links
.vector-legend {
  position: absolute;
  left: 18px;
  bottom: 64px;
  z-index: 900;
  display: flex;
  flex-direction: column;
  gap: 0.5rem;
  pointer-events: none;
}

.vector-legend-card {
  background: rgba(17, 24, 39, 0.92);
  color: #f8fafc;
  border: 1px solid rgba(148, 163, 184, 0.35);
  box-shadow: 0 16px 36px rgba(15, 23, 42, 0.28);
  padding: 0.5rem 0.7rem;
  border-radius: 10px;
  width: 12rem;
}

.vector-legend-title {
  font-weight: 700;
  font-size: 0.72rem;
  color: rgba(191, 219, 254, 0.9);
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.vector-legend-property {
  color: rgba(226, 232, 240, 0.72);
  text-transform: uppercase;
  letter-spacing: 0.04em;
  font-size: 0.68rem;
  margin-bottom: 0.3rem;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.vector-legend-gradient {
  display: block;
  width: 100%;
  height: 10px;
  border-radius: 3px;
}

.vector-legend-labels {
  display: flex;
  justify-content: space-between;
  margin-top: 0.2rem;
  font-size: 0.7rem;
  font-variant-numeric: tabular-nums;
}
</style>
